...
RS-BASIC aims for compatibility with a time when BASIC usually came on ROM.
...
RS-BASIC has all the quirks of the most popular ROM BASICs. This appendix
...
RS-BASIC reseeds the random number generator with good entropy on every run,
...
in RS-BASIC so if you see something similar to the following, you can
delete it.
...
To get a single keypress and store it in A$, RS-BASIC uses the newer style:
```text
A$ = INKEY$
```
...
RS-BASIC does not support sound. Your terminal might beep with:
```text
PRINT CHR$(7)
```
...
Also, RS-BASIC arrays are sparse so by simply not using 0 it won't be allocated.
