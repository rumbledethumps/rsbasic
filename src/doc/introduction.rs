...
RS-BASIC is interactive just like it was back in 1964 when the idea of an
ordinary person sitting in front of a terminal and directly interacting
with a computer was revolutionary. A primary design goal for RS-BASIC
...
Type CTRL-D to exit RS-BASIC. Stop a running BASIC program with CTRL-C.

How to run command line programs on every operating system RS-BASIC
...
When you see the `READY.` prompt, RS-BASIC is ready to accept a statement.
...
operating system when RS-BASIC was started. The `NEW` command erases the
program in memory.
...
Line 30 was intentionally wrong to demonstrate a compiler error. RS-BASIC
...
reference material covering everything RS-BASIC can do.
